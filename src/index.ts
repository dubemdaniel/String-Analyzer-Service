...
const storage: Map<string, StringData> = new Map();
...
function analyzeString(value: string): StringData {
...
app.get('/strings/:string_value', (req: Request, res: Response) => {
...
app.get('/strings', (req: Request, res: Response) => {
  ...
  if (contains_character) filters_applied.contains_character = String(contains_character);
...
app.delete('/strings/:string_value', (req: Request, res: Response) => {
  const stringValue = decodeURIComponent(req.params.string_value!);
  const hash = computeSHA256(stringValue);
